...
namespace artkost\yii2\taxonomy\admin\controllers;
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->validate()) {
                if ($model->save(false)) {
                    ...
                    Yii::$app->session->setFlash('danger', Manager::t('admin', 'Failed to create a vocabulary'));
                    return $this->refresh();
                }
            ...
            if ($model->validate()) {
                if ($model->save(false)) {
                    ...
                    Yii::$app->session->setFlash('danger',
                        Manager::t('admin', 'Failed to update vocabulary {name}', ['{name}' => $model->name]));
                    return $this->refresh();
                }
            } elseif (Yii::$app->request->isAjax) {
                Yii::$app->response->format = Response::FORMAT_JSON;
